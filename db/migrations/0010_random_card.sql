-- Draw a random real UFC event (card), weighted toward recent + numbered events,
-- and return its top ~12 most famous (by Elo) playable fighters with full attrs.
-- Powers the card-based draft (sortear um card -> escolher um lutador dele).
  ...
  WITH chosen AS (
    ...
    ORDER BY (-ln(random()) / GREATEST(0.1,
      (CASE
         ...
         ELSE 0.6 END)
      * (CASE WHEN e.ufc_number IS NOT NULL THEN 1.5 ELSE 1.0 END)
    )) ASC
    LIMIT 1
  ),
  roster AS (
